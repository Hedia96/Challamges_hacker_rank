...
//    char size;
    ...
    FILE * fp;
    char * filename ="E:\\ITI39\\Embedded sessions\\questions_valeo\\Interview_Answer_Codes\\BiggerisGreater\\input.txt";
   fp=fopen(filename,"r");
   fgets(size,28,fp);
   if(fp)
size_int=atoi(size);
...
*/
   int size_int;
    ...
    int j;
   scanf("%d",&size_int);
   char ** ARR_2D=(char **)malloc(size_int*sizeof(char *));
  //char * ARR_2D=(char* )malloc(size*28*sizeof(char));
  // access the *(arr+(index_row*col)+index_col)
   for(i=0;i<size_int;i++)
  // i=2;
    {
        *(ARR_2D+i)=(char* )malloc(28*sizeof(char));

        scanf("%s",(ARR_2D+i));
     //   fgets((ARR_2D+i),28,fp);
       // printf("%s\n",ARR_2D+i);
        printf("%s\n",next_permutation(ARR_2D+i));
    }
   // fclose(fp);
